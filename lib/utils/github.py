...
def get_repos(predicate=None):
    ...
    """
    endpoint = '{}?per_page=200'.format(GH_API_ORG_REPOS)
    entities = []
    _get_entities(endpoint, lambda i: i['name'].startswith(".") == False, entities)

    return collections.OrderedDict([(i['name'], GitHubRepo(i)) \
           for i in sorted(entities, key=lambda i: i['name'])])
